...
export default function Product() {
  ...
  function handleChange(event) {
    setEdatafilter(data.filter(product => product.brand.toLowerCase().includes(event.target.value) && product.category === productType))
  }

  const data = useContext(ProductsData);
  const [productType, setproductType] = useState('smartphones')
  const [eDatafilter, setEdatafilter] = useState(data)
  const [toggleState, setToggleState] = useState(1);
  const [productCount, setproductCount] = useState(productCount2);
  ...
  const toggleTab = (index) => {
    ...
    if (index == 4) {
      ...
      setEdatafilter(data.filter(product => product.category === "smartphones"))
      productCount2 = []
      for (let i in eDatafilter) {
        productCount2.push(parseInt(i) + 1)
      }
      ...
      setEdatafilter(data.filter(product => product.category === "smartwatch"))
      productCount2 = []
      for (let i in eDatafilter) {
        productCount2.push(parseInt(i) + 1)
      }
